...
MODEL = "claude-opus-4-5"  # Change to a cheaper model if you want, e.g. claude-sonnet-4-5
...
if generate:
    ...
    if target_users:
        ...
        context_lines.append("**Target users:** Not specified — infer from product context and add a note in the audit.")
    if audience:
    ...
    user_text = (
        ...
        "(see below). Skip Step 0a, 0b, 0c and proceed directly to Phase 1 — the written audit — "
        "in the chosen language. Do not ask any more clarifying questions unless critical "
...
if st.session_state.audit_text:
    ...
    if st.button("Generate deck →", type="primary"):
        ...
        deck_user = (
            ...
            "For each finding, match the screenshot reference in the audit to the "
            "correct uploaded file.\n\n"
            "Write a Python script that builds the deck using python-pptx and the "
            "helpers in scripts/pptx_helpers.py from the deck skill. Save the output "
            "as `output.pptx`."
        )

        st.info(
            "Note: For this MVP, the deck skill is provided as reference. "
            "In v2, the app will execute the Python script server-side. For now, "
            "the audit text above is your deliverable — copy it into your existing "
